/**
 * @description:
 * Class for trailing stop trading (for in to out positioning)
 *
 * 
 * This class implements a configurable trailing stop like trading to go out of a position
 *
 * 
 * @author Atzen
 * @version 0.5.0
 ...
 * 02-Apr-2017 : renamed PlThresholdOut -> PlTrailingStopOut
 */
 ...
 * @type {Object}
 */
PlTrailingStopOut.ConfigDefault = {
    id: 'undefined',
    name: 'undefined',
    ...
    thresholdValue: 0,
    thresholdExceedings: 1, // plugin only sets an action (buy/sell) when the threshold is exceeded this number of times in a row
...
export function PlTrailingStopOut(logger) {
     ...
     * @type {Object}
     */
    var _config = Object.assign({}, PlTrailingStopOut.ConfigDefault);
    ...
    var _checkConfig = function() {
        ...
        if (isNaN(_config.thresholdValue)) return false;
        if (_config.thresholdValue < 0) return false;

        if (_config.thresholdType === 'percentage') {
            if (_config.thresholdValue > 100) return false;
        }

        if (isNaN(_config.thresholdExceedings)) return false;
        if (_config.thresholdExceedings < 1) return false;
    ...
    this.setConfig = function(configuration) {
        _logPreMsg = 'PlTrailingStopOut ' + configuration.id + ': ';
    ...
    this.getConfig = function() {
        ...
        return [
            ...
            { title: 'Threshold', value: _config.thresholdValue },
    ...
    this.update = function(price) {
        if (_active) {
            ...
            if (_position === 'long') {
                ...
                if (diff < -_config.thresholdValue) _thrshldCnt++;
                else _thrshldCnt = 0;

                if (_thrshldCnt >= _config.thresholdExceedings) {
    ...
    this.getInstInfo = function() {
        ...
        return { id: _config.id, name: _config.name, type: "PlTrailingStopOut" };
    }

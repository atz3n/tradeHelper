...
var deactivateStrategies = function(){
  Strategies.find().forEach(function(item) {
    Strategies.update({ _id: item._id }, { $set: { active: false } });
    Strategies.update({ _id: item._id }, { $set: { paused: false } });
  });	
}
...
Accounts.onCreateUser(function(options, user) {
  ...
  Settings.insert({ enNotification: 'true', dummy: 'false', ownerId: user._id, modifiedBy: user._id });
  return user;
});
